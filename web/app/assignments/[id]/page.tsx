...
import { ExternalLink, ChevronDown, ChevronUp, Clock, Trash2, CheckCircle2, FilePlus } from "lucide-react";
...
export default function AssignmentDetail() {
  ...
  useEffect(() => {
    api.google.status(getUserId()).then((r) => setGoogleConnected(r.authorized)).catch(() => setGoogleConnected(false));
  }, []);
  ...
  async function handleCreateDocument() {
    ...
    try {
      const updated = await api.assignments.createDocument(id);
      setAssignment(updated);
    } catch (e) {
      setDocError(e instanceof Error ? e.message : "Could not create Google Doc");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8 min-w-0">
        <div className="lg:col-span-2 space-y-6 min-w-0">
          ...
          {!assignment.document_url && !isCompleted && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-amber-900">No Google Doc linked</p>
                  <p className="text-xs text-amber-800 mt-1">
                    {googleConnected
                      ? "Create a doc to track progress in Google Docs with the extension."
                      : "Connect Google first, then create a doc for this assignment."}
                  </p>
                  {docError && (
                    <p className="text-xs text-red-700 mt-2">{docError}</p>
                  )}
                </div>
                {googleConnected ? (
                  <button
                    ...
                    disabled={actionLoading}
                    className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-scaffold-500 rounded-lg hover:bg-scaffold-600 disabled:opacity-60 shrink-0"
                  >
                    <FilePlus size={16} />
                    Create Google Doc
                  </button>
                  ...
                  <a
                    href={api.google.authorizeUrl(getUserId())}
                    className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-scaffold-700 bg-white border border-scaffold-200 rounded-lg hover:bg-scaffold-50 shrink-0"
                  >
                    Connect Google
                  </a>
                )}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 overflow-hidden min-w-0">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">
              Prompt
            </p>
            <PromptBody text={assignment.prompt} />
          </div>
